  ...
  <div class="login-container">
    <div class="login-form-container">
      <div class="login-welcome">
        <span class="welcome">用户注册</span> <span class="btn-register fRight">已有账号，<a>立即登录</a></span>
      </div>
      <el-row>
...
export default {
  ...
  created () {
    const vm = this
    vm.homeService = new HomeService({ $axios: vm.$axios, app: { $cookies: vm.$cookies } })
  },
  methods: {
    registerUser () {
      const vm = this
      ...
      this.$refs.form.validate((valid) => {
        ...
        vm.homeService.register(tempParams).then((res) => {
          vm.$message({
            message: '注册成功',
            type: 'success'
          })
    ...
    fetchAuthCode () {
      const vm = this
      if (vm.form.mobile && vm.form.mobile.length !== 11) {
        vm.$message({
          message: '请输入正确的手机号',
          type: 'error'
        })
      ...
      vm.homeService.sendValidCode({ mobile: vm.form.mobile }).then((res) => {
        if (vm.countDown > 0) {
          return
        }
        vm.countDown = 59
